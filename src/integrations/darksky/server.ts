...
export function init(server: express.Express) {
  ...
  server.get('/api/darksky/forecast/:integrationId', async (req: express.Request, res: express.Response) => {
    ...
    const url = `https://api.darksky.net/forecast/${apiKey}/${latitude},${longitude}?units=${units}`;
