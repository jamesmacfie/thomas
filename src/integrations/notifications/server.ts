...
export function init(server: express.Express) {
  ...
  (server as any).ws('/api/notifications/:deviceId', function(ws: WebSocket, req: express.Request) {
  ...
  server.post('/api/notification/:deviceId', async (req: express.Request, res: express.Response) => {
    const deviceId = req.params.deviceId;
