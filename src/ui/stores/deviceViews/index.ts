...
export default class Store {
  ...
  fetchAll = async ({ deviceId }: { deviceId: number | string }) => {
    ...
    const deviceViews = await fetch(`${api_url}/api/device/${deviceId}/views`).then(res => res.json());
  ...
  insert = async (values: { viewId?: number; name: string; icon: string }) => {
    ...
    const deviceView = await fetch(`${api_url}/api/device/${devicesStore.getDeviceId()}/view`, {
      method: 'POST',
      headers: {
  ...
  update = async (values: { viewId: number; name: string; icon: string }) => {
    ...
    const deviceView = await fetch(`${api_url}/api/device/view/${values.viewId}`, {
      method: 'PATCH',
      headers: {
        'content-type': 'application/json'
      },
    ...
    try {
      await Promise.all(
        updates.map(u => {
          ...
          fetch(`${api_url}/api/device/view/${u.deviceViewId}`, {
            method: 'PATCH',
